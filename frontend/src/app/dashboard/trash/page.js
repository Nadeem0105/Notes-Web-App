...
import { fetchTrashNotes, recoverNote, permanentDeleteNote } from '@/api/notesApi';
...
export default function TrashPage() {
  ...
  const handleRecover = async (id, e) => {
    e.stopPropagation();
  ...
  return (
    <div className="max-w-[1280px] mx-auto p-[var(--spacing-margin-desktop)] min-h-full">
      <div className="mb-8 flex items-center justify-between">
        <p className="font-body-lg text-[length:var(--text-body-lg)] text-[color:var(--color-on-surface-variant)]">Items here will be permanently incinerated after 30 days.</p>
      ...
      {fetchError ? (
        <div className="text-center py-20 flex flex-col items-center">
          ...
          <h2 className="font-headline-md text-[length:var(--text-headline-md)] text-[color:var(--color-error)]">Cannot connect to the Incinerator.</h2>
        ...
        <div className="text-center py-20 text-[color:var(--color-outline)] font-headline-sm">The incinerator is empty.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[var(--spacing-gutter)]">
          {notes.map((note, index) => (
            <article key={note._id} className="bg-[color:var(--color-surface-container-low)] border border-[color:var(--color-outline-variant)] p-[var(--spacing-card-padding)] relative group hover:-translate-y-1 hover:border-[color:var(--color-outline)] transition-all duration-200 shadow-[2px_2px_0px_0px_rgba(45,45,45,0.1)] flex flex-col min-h-[250px]">
              ...
              <div 
                ...
                style={{
                  ...
                  border: '3px solid rgba(180, 50, 50, 0.4)',
                }}
              >
                TRASHED
              </div>
              
              <div className="relative z-10 flex flex-col h-full opacity-60 grayscale group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                ...
                <div className="mt-4 pt-4 border-t border-[color:var(--color-outline-variant)] flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={(e) => handleRecover(note._id, e)} className="font-headline-sm text-[length:var(--text-headline-sm)] text-[color:var(--color-primary)] hover:underline flex items-center gap-1 z-30">
                    <span className="material-symbols-outlined text-sm">settings_backup_restore</span> Recover
                  </button>
                  <button onClick={(e) => handlePermanentDelete(note._id, e)} className="font-headline-sm text-[length:var(--text-headline-sm)] text-[color:var(--color-error)] hover:underline flex items-center gap-1 z-30">
